...
// Simplified database operations interface for projects
export type DBOperations<Schema> = {
  [TableName in keyof Schema]: {
    ...
    findUnique: (where: { id: string }) => Promise<Schema[TableName] | null>;
    update: (params: {
      where: { id: string },
      data: Partial<Schema[TableName]>
    }) => Promise<Schema[TableName]>;
    delete: (where: { id: string }) => Promise<Schema[TableName]>;
    deleteMany?: (params?: {
      where?: Partial<Schema[TableName]>
    }) => Promise<Schema[TableName][]>;
    search?: (

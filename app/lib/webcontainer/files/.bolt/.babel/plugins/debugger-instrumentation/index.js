    ...
    visitor: {
      Program: {
        enter(programPath, state) {
          ...
          programPath.get('body').forEach((p) => {
            if (p.isIfStatement() && p.node._generated_by_babel_plugin_time_travel_stub) {
              hasStub = true;
            }
          });
      ...
      CallExpression(path) {
        ...
        if (name === 'describe') {
          ...
          if (!titleNode?.isStringLiteral()) {
            return;
          }

          const suiteName = sanitizeForPath(titleNode.node.value);

          if (!fnNode || !(fnNode.isFunctionExpression() || fnNode.isArrowFunctionExpression())) {
            return;
          }
      ...
      Statement(path, state) {
        if (STATEMENTS_TO_SKIP.has(path.node.type)) {
          return;
        }
      ...
      // -------------- function entry --------------
      Function(path, state) {
        if (path.node._generated_by_plugin_) {
          return;
        }

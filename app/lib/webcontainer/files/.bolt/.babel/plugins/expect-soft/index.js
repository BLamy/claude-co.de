  ...
  return {
    ...
    visitor: {
      CallExpression(path) {
        ...
        if (!callee.isIdentifier({ name: 'expect' })) {
          return;
        }
        ...
        if (stmtPath) {
          ...
          const hasHardEscape = leadingComments.some((c) => c.value.trim() === 'hard');
        ...
        callee.replaceWith(t.memberExpression(t.identifier('expect'), t.identifier('soft')));
      },
    },
  };
}

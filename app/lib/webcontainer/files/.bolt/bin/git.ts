...
async function main() {
  ...
  try {
    switch (command) {
      ...
      case 'remote':
        const remoteAction = args[0]; // add, remove, get-url, etc.
        const remoteName = args[1];
        const remoteUrl = args[2];
        
        if (remoteAction === 'add' && remoteName && remoteUrl) {
          try {
            await git.addRemote({
              fs,
              dir,
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            console.error('Failed to add remote:', err.message);
          }
        } else if (remoteAction === 'get-url' && remoteName) {
          try {
          ...
          try {
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            console.error('Failed to list remotes:', err.message);
          }
        }
        break;
...
Available commands:
  ...
  push <url> [<remote>] [<branch>]  Update remote refs along with associated objects
  ...
  remote [add|get-url] [<name>] [<url>]  Manage set of tracked repositories
